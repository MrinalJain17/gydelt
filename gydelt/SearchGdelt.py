...
def read_from_file(path, seperator='\t', parse_dates=False, encoding=None, header=0):
...
def fire_query(project_id, fields_required=['DATE', 'DocumentIdentifier', 'Themes', 'Locations', 'Persons', 'Organizations', 'V2Tone'],
			   is_search_criteria=False, get_stats=True, auth_file='', search_dict = {}, limit=None, save_data=True, is_web_ui=False):
	...
	Args:
		...
		fields_required (list) : default ['GKGRECORDID', 'CAST(DATE AS STRING) as DATE', 'DocumentIdentifier', 'Themes', 'Locations', 'Persons', 'Organizations', 'V2Tone', 'AllNames']
		...
		auth_file (str) : default '' (empty)
			The path of the authorization file received from BigQuery. (For further details, refer 'Setup before using gydelt')
			
			->If path is provided, stats will be displayed before executing the query
			->If not, a message will be displayed and the user will be asked whether or not to proceeds
			
		search_dict (dict) : default {} (empty)
		
			-> If dictionary is passed, it is CASE-SENSITIVE. Therefore, give the values in proper casing.
			...
			-> If No keywords is to be added in a certain part, leave it empty (BUT, DO NOT miss the semi-colons)
		
			Eg - 
			
			-> {'Persons': 'P1;P2,P3;P4', 'Organizations': ';O1,O2;'}
		...
		is_web_ui (boolean) : default True
			Used by the web UI to produce the required output/message
	...
	if (is_search_criteria):
		...
		query = query + create_query(console_input=is_console_input, search_dict=search_dict)
	else:
		query = query + 'True'
	if limit != None:
	...
	if get_stats == True:
		if auth_file != '':
			message = get_query_stats(query=query, is_auth_file=True, auth_file=auth_file)
		else:
			message = get_query_stats(query=query, is_auth_file=False)
		if(is_web_ui):
			return message
		else:
...
def save_data_frame(data_frame, path=None, seperator='\t', index=False):
	...
	Args:
		...
		path (str) : default None
			...
			It should be of the following format - <dir>/<sub-dir>/.../<file-name.csv> (recommended)
...
def get_query_stats(query, is_auth_file, auth_file=''):
...
def create_query(console_input, search_dict):
